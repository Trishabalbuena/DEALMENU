...
static handleCourseChange(selectedCourse) {
    ...
    static renderTradeOffers() {
        ...
        container.innerHTML = trades.map(t => {
            const statusBadge = t.status === 'accepted' ? '<span style="background:#28a745; color:#fff; padding:4px 8px; border-radius:4px; font-size:0.8rem;">Accepted</span>' : 
                               t.status === 'counter' ? '<span style="background:#ffc107; color:#000; padding:4px 8px; border-radius:4px; font-size:0.8rem;">Counter Offer</span>' : 
                               '<span style="background:#007bff; color:#fff; padding:4px 8px; border-radius:4px; font-size:0.8rem;">Pending</span>';
            return `
            <div class="item-card" style="padding:12px; border-left:4px solid ${t.status === 'accepted' ? '#28a745' : t.status === 'counter' ? '#ffc107' : '#007bff'};">
                <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:8px;">
                    <h4 style="margin:0; flex:1;">${t.item_title}</h4>
                    ${statusBadge}
                </div>
                <p style="color:var(--gray); font-size:0.9rem; margin:4px 0;">From: <strong>${t.offered_by_name}</strong></p>
                <p style="font-size:0.9rem; margin:4px 0;">Offering: ${t.offered_items.join(', ')}</p>
                <button class="btn-primary" onclick="AppManager.openTradeOffer(${t.id})" style="margin-top:8px; width:100%; padding:10px;">View Details</button>
            </div>
            `;
        }).join('');
    }
    ...
    static renderMyTradeItems() {
        ...
        container.innerHTML = items.map(item => `
            <div class="item-card" style="padding:12px;">
                ...
                <p style="font-size:0.9rem; color:var(--primary);"><strong>Looking for:</strong> ${item.looking_for}</p>
                <div style="display:flex; gap:8px; margin-top:8px; font-size:0.8rem;">
                    <span style="background:var(--light); padding:4px 8px; border-radius:4px;">${item.condition}</span>
                    ${item.cash_with_trade ? `<span style="background:var(--light); padding:4px 8px; border-radius:4px;">+₱${item.cash_with_trade}</span>` : ''}
                </div>
            </div>
        `).join('');
    }
...
static loadMarketplace() {
    ...
    static renderItems(items, container) {
        ...
        items.forEach(item => {
            ...
            itemCard.innerHTML = `
                <!-- Added onclick to the image container -->
                <div class="item-image" style="cursor: pointer;" onclick="AppManager.openProductPreview(${item.id})">
                    ...
                    <!-- Wishlist button (stop propagation prevents opening modal) -->
                    <button class="item-wishlist" data-item-id="${item.id}" onclick="event.stopPropagation();">
                        <i class="${isInWishlist ? 'fas' : 'far'} fa-heart"></i>
                    </button>
                </div>
                
                <div class="item-info">
                    <!-- Added onclick to title too -->
                    ...
                    <!-- BUTTONS FIX (Included your previous fix here) -->
                    <div class="item-actions" style="display: flex; gap: 10px; height: 48px; margin-top: 15px;">
                        <button class="btn-chat" data-item-id="${item.id}" style="flex: 1; height: 100%; display: flex; align-items: center; justify-content: center; border-radius: 8px;">
                            <i class="fas fa-comment"></i> Chat
                        </button>
                        ${isSeller ? `
                        <button class="btn-sold mark-sold" data-item-id="${item.id}" ${item.status === 'sold' ? 'disabled' : ''} style="flex: 1; height: 100%; display: flex; align-items: center; justify-content: center; border-radius: 8px; white-space: nowrap; padding: 0 5px;">
    ...
    static filterProfileItems(filterType) {
        ...
        // Logic: Get Items
        const allItems = this.db.getUserItems(user.id);
        let displayItems = [];

        if (filterType === 'all') {
            displayItems = allItems;
        } else if (filterType === 'available') {
            displayItems = allItems.filter(i => i.status !== 'sold');
        ...
        if (container) {
            if (displayItems.length === 0) {
                ...
                this.renderItems(displayItems, container);
            }
        }
    }
